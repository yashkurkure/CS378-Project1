...
You are given a working project where some pieces are fully implemented
and others are stubbed out with `TODO` comments and `throw
UnimplementedError()`. It always compiles and runs, so you have a working
program throughout:

- `lib/src/cell.dart`, `lib/src/grid.dart`, and
  `lib/src/cellular_automaton.dart` — fully given; you shouldn't need to
  modify them.
- `lib/src/rules.dart` — `ConwayRules` is fully given as a worked example;
  `HighLifeRules`, `DayAndNightRules`, and `SeedsRules` are yours to
  implement, following its pattern.
- `lib/src/edge_policies.dart` — `ToroidalEdges` is given; `WalledEdges`
  is yours to implement.
- `lib/src/rendering.dart` — `AsciiRenderable` is given; `ColorRenderable`
  is yours to implement.
- `lib/src/stats.dart` — `Trackable` is entirely yours to implement.
- `bin/cellular_automaton.dart`, a complete command-line application that
  ...
  species out of the mixins above, and handles argument parsing, seeding,
  and printing the animation to the terminal. You should not need to
  modify this file.

The project uses the `args` and `test` packages for the command-line
functionality and the tests, respectively. You will need to add these to
the project yourself using `dart pub add`:
...
3. Copy the provided starter files into place: replace the generated
   ...
   `lib/src/` files, and copy the given `test/` folder in as well. There
   is no `bin/` folder yet — create one yourself at the project root and
   copy `bin/cellular_automaton.dart` into it.
